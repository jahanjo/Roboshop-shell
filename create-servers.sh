...
##### Change these values ###
ZONE_ID="Z03084772QJDKOWQFLMT7"
DOMAIN="jahanjo.online"
SG_NAME="allow-all"
env=dev
#############################
...
create_ec2() {
  ...
  aws route53 change-resource-record-sets --hosted-zone-id ${ZONE_ID} --change-batch files:///tmp/record.json 2>/dev/null
  if [ $? -eq 0 ]; then
